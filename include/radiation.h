...
vec3d interpolate(const vec3d points[3], double t) {
  vec3d interpolatedPoint;
  interpolatedPoint.x =
      (1 - t) * points[0].x + t * ((1 - t) * points[1].x + t * points[2].x);
  interpolatedPoint.y =
      (1 - t) * points[0].y + t * ((1 - t) * points[1].y + t * points[2].y);
  interpolatedPoint.z =
      (1 - t) * points[0].z + t * ((1 - t) * points[1].z + t * points[2].z);
...
bool doesRayIntersectSurface(const vec3d& point1, const vec3d& point2,
  ...
  // the ray misses the triangle and we can skip the rest of this test for this
  // triangle (continue) and move on to the next triangle.
  double u = f * dot(s, h);
  if (u < 0.0 || u > 1.0) {
    return false;
  }
...
bool isObstructed(const vec3d& point1, const vec3d& point2,
  ...
  // If we've checked all the surfaces and found no intersections, then there is
  // no obstruction
  return false;
}
...
double calculatePointToPointViewFactor(
    const vec3d& point1, const vec3d& normal1, const vec3d& point2,
    const vec3d& normal2, const std::vector<Surface>& allSurfaces) {
...
double calculateViewFactor(const Surface& surface1, const Surface& surface2,
  ...
  for (int i = 0; i < numPoints; ++i) {
    ...
    vec3d point1 =
        interpolate(surface1.points, i / static_cast<double>(numPoints));
    ...
    for (int j = 0; j < numPoints; ++j) {
      // Calculate the position and normal of this point
      vec3d point2 =
          interpolate(surface2.points, j / static_cast<double>(numPoints));
...
double calculateTotalRadiativeHeatTransfer(
    const Surface& surface, const std::vector<Surface>& allSurfaces) {
